...
resource "spectrocloud_cluster_aws" "aws-cluster" {
  ...
  name             = "linus3-aws-cluster" #Enter your unique AWS Cluster name
  ...
  cluster_profile {
    id = var.deploy-aws-novar && var.deploy-aws-var ? resource.spectrocloud_cluster_profile.aws-profile-var[0].id : resource.spectrocloud_cluster_profile.aws-profile-novar[0].id
  }

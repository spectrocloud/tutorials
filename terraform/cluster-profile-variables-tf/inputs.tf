...
variable "aws-cloud-account-name" {
  ...
  validation {
    condition     = var.deploy-aws-novar ? var.aws-cloud-account-name != "REPLACE ME" && var.aws-cloud-account-name != "" : true
    error_message = "Provide the correct AWS cloud account name."
  }
}

variable "deploy-aws-novar" {
  type        = bool
...
variable "aws-region" {
  ...
  validation {
    condition     = var.deploy-aws-novar ? var.aws-region != "REPLACE ME" && var.aws-region != "" : true
...
variable "aws-key-pair-name" {
  ...
  validation {
    condition     = var.deploy-aws-novar ? var.aws-key-pair-name != "REPLACE ME" && var.aws-key-pair-name != "" : true
...
variable "aws_control_plane_nodes" {
  ...
  validation {
    condition     = var.deploy-aws-novar ? length(var.aws_control_plane_nodes.availability_zones) > 0 && !contains(var.aws_control_plane_nodes.availability_zones, "REPLACE ME") : true
...
variable "aws_worker_nodes" {
  ...
  validation {
    condition     = var.deploy-aws-novar ? length(var.aws_worker_nodes.availability_zones) > 0 && !contains(var.aws_worker_nodes.availability_zones, "REPLACE ME") : true
    error_message = "The availability_zones parameter must be set correctly"
  }
}
...
variable "db_password" {
  ...
  validation {
    condition     = var.deploy-aws-novar || var.deploy-azure || var.deploy-gcp ? var.db_password != "REPLACE ME" && var.db_password != "" : true
...
variable "auth_token" {
  ...
  validation {
    condition     = var.deploy-aws-novar || var.deploy-azure || var.deploy-gcp ? var.auth_token != "REPLACE ME" && var.auth_token != "" : true

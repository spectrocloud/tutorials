...
run "verify_aws_kubecost" {
  ...
  assert {
        condition     = length(spectrocloud_cluster_profile.aws-profile-kubecost) == 1
        error_message = "No AWS cluster profile with Kubecost was created"
    }
